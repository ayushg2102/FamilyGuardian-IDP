...
import { UserOutlined, LogoutOutlined, MenuOutlined, DownOutlined, DashboardOutlined, UnorderedListOutlined } from '@ant-design/icons';
...
const AppLayout: React.FC = () => {
  ...
  const sideMenuItems = [
    ...
    },
    {
      ...
      onClick: () => { setDrawerOpen(false); navigate('/requests'); },
    },
    {
      key: 'profile',
      icon: <UserOutlined />,
    ...
    <Layout style={{ minHeight: '100vh', background: '#f5f5f5' }}>
      <Header
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1 }}>
          {/* Hamburger Icon */}
          <MenuOutlined style={{ fontSize: 22, cursor: 'pointer', marginRight: 12 }} onClick={() => setDrawerOpen(true)} />
          <div  style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 220 }}>
            <img 
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', marginLeft: 24 }}>
        ...
        <Space size="large">
          ...
          <Dropdown menu={{ items: userMenuItems }} trigger={['click']}>
            <div style={{ cursor: 'pointer', padding: '4px 8px', borderRadius: '6px', position: 'relative', display: 'flex', alignItems: 'center', minWidth: 180 }} className="user-dropdown">
              ...
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', lineHeight: 1, minWidth: 90 }}>
                  <Text strong style={{ fontSize: '18px', display: 'block', lineHeight: '22px', margin: 0, padding: 0 }}>{user?.name}</Text>
                  <Text style={{ fontSize: '15px', color: '#666', lineHeight: '18px', margin: 0, padding: 0 }}>{user?.department}</Text>
                </div>
                {/* Chevron down icon */}
        ...
        <Drawer
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <Avatar size={48} src={user?.avatar} icon={<UserOutlined />} style={{ border: '2px solid #00b894' }} />
              <div>
        ...
        >
          <Menu
            mode="inline"
            style={{ border: 'none', fontSize: 16, fontWeight: 500, background: '#f7fafc', paddingTop: 16 }}
            items={sideMenuItems}
            onClick={({ key }) => {
              const item = sideMenuItems.find(i => i.key === key);
              if (item && item.onClick) item.onClick();
            }}
            defaultSelectedKeys={[window.location.pathname.includes('dashboard') ? 'dashboard' : window.location.pathname.includes('requests') ? 'requests' : 'profile']}
          />
          <div style={{ position: 'absolute', bottom: 24, left: 0, width: '100%', textAlign: 'center', color: '#bbb', fontSize: 13 }}>
